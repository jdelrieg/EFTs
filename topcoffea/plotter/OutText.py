...
class OutText:
  ...
  def write(self):
    ...
    if os.path.isfile(filename) and (self.mode == 'new' or self.mode == 'w'):
      if self.verbose: print('[INFO] %s exists!! moving to .bak...'%filename)
  ...
  def __init__(self, path = 'temp/', outname = 'out', mode = 'new', textformat = 'txt', doPrint=False):
