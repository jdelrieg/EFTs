...
def GetHisto(path, hname, categories={}, group=None, integrate=None, rebin=None):
...
def GetHistoBIS(path,pesos, hname, categories, group=None, integrate=None, rebin=None):
  ''' Get a histogram from a pkl file (standard output from coffea analysis) '''
  histogramas={}                             #Defino un diccionario donde voy a almacenar los histogramas de ese conjunto
  for d in range(len(path)):
	  print('Opening path: ', path[d])
	  h={}
	  hists={}                              #Necesitamos otro diccionario para cada .pkl.gz
	  with gzip.open(path[d]) as fin:
		  hin=pickle.load(fin)
		  print('>> looking for histograms...')
		  for k in hin.keys():
			  if k in hists: hists[k]+=hin[k]
			  else:          hists[k]=hin[k]
	  histogramas[d]=hists[hname]             #En cada key del diccionario principal almacenamos el diccionario del .pkl.gz en cuestion
  ...
  for d in range(1,len(path)):
	  histograma=histograma+histogramas[d]   
  h=histograma
  for cat in categories.keys():
	  h = h.integrate(cat, categories[cat])
  if group is not None:
...
class plotter:
    ...
    for path in listpath:
      with gzip.open(path) as fin:
        ...
        for k in hin.keys():
          ...
          #print('Adding: ', hin[k], ', sample = ', hin[k].identifiers('sample'))
          if not 'appl' in [x.name for x in hin[k].sparse_axes()]: continue
  ...
  def AddExtraBkgHist(self, h, add=False):
    ...
    for pr in h.identifiers('process'): 
      self.bkglist.append(pr.name)
    if add:
      return self.AddExtraBkgToHisto()

  def ResetExtraBkg(self):
    self.extraBkg = None
    self.bkglist = self.bkglistOrig.copy()
  ...
  def GetYields(self, var='counts', cat=None, pr=None, doErr=False):
    sumy = 0; sumerr = 0;
    dicyields = {}
    dicerrors = {}
